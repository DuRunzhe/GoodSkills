...
**模板位置**:`assets/` 目录(实际文件名:overview-map-template.html / guide-template.md / trip.kml.template.xml / pois.schema.json)。
...
5. 调 `screenshot_html.py` 截图自检
6. 写攻略长文 `_posts/YYYY-MM-DD-<slug>.md`
7. 提交推送 + 通知(用 `git push` 或 `openclaw message send`)
