...
| 4 | 数据约定已对齐(§4.6 标签/精度/输入形式) | ✅ |
| 5 | 部署路径通畅(cp output → ~/blog/ → git push → openclaw notify) | ✅ |

**自检通过后,LLM 即可按 §3.1 生命周期执行 5 步推理 + 按 §4.5 质量门(10 项)自检 + 按 §4.6 数据约定渲染 + 按 Step 5 部署清单推送。**
