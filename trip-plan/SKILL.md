...
> 💡 **POI 建议带 `city_adcode`**（高德 6 位城市码，如北京 110000）和 **`city_name`**（中文名，如北京/承德/克什克腾旗）：
> - 🔍 搜索链接用 `keyword=城市名+地点名` 限定城市（高德 `city` 参数实测不生效，城市名拼进 keyword 才有效）
> - 📋 复制按钮复制「城市名+地点名」（如「围场七星湖」），名称已含城市前缀（围场县城午餐/太仆寺旗）不重复拼接
> - 缺省时搜索不带城市限定、复制按钮退化为纯地点名，导航/标记不受影响

> 🎫 **景点建议带门票信息**（`ticket_price` / `ticket_open` / `ticket_free`）：
> - 卡片 info 下方渲染橙色左边框 ticket 行：🎫 门票 / 🕐 开放时间 / 🆓 免票政策
> - 综合地图 popup 同样显示
> - 例：`ticket_price: "120 元/人（通票，3 日有效）"`、`ticket_open: "旺季 7:00-19:00"`、`ticket_free: "1.2m 以下儿童、70 岁以上老人、现役军人免票；学生半价"`
> - 无门票/免费景点写 `免费` 或省略；validate.py 会对 tag=attract 缺门票信息给出 warning
