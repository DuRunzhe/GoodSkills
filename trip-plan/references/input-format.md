...
| `pois[].city_name` | △ | **推荐**。城市中文名(如 北京/承德/克什克腾旗/围场)。搜索链接 `keyword=城市名+地点名` 用它拼搜索词（高德 city 参数不生效，城市名拼进 keyword 才有效）；📋 复制按钮用它复制「城市名+地点名」。名称已含城市前缀（如「围场县城午餐」「太仆寺旗」）时生成脚本自动跳过拼接 |
| `pois[].ticket_price` | △ | 门票价格（景点建议带）。如 `120 元/人（通票，3 日有效）` / `免费` / `门票 120 + 观光车 40` |
| `pois[].ticket_open` | △ | 开放/营业时间。如 `旺季 7:00-19:00` / `全天开放` |
| `pois[].ticket_free` | △ | 免票/优惠政策。如 `1.2m 以下儿童、70 岁以上老人、现役军人免票；学生半价` |
